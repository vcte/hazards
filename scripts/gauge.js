function Gauge(placeholderName)
{
    var configuration = 
	{
		size: 240,
	...
	configuration.min_red = 90;
	configuration.greenZones = [{ from: 0, to: configuration.min_yellow }];
	configuration.yellowZones = [{ from: configuration.min_yellow + 1, to: configuration.min_red }];
	configuration.redZones = [{ from: configuration.min_red + 1, to: configuration.max }];
	...
	this.render = function(value)
	{
		...
		var yellow_pt = this.valueToPoint(configuration.min_yellow, 0.63);
		this.body.append("svg:text")
		 			.attr("x", yellow_pt.x)
		 			.attr("y", yellow_pt.y)
		 			.attr("dy", fontSize / 3)
		 			.attr("text-anchor", "start")
		 			.text(configuration.min_yellow)
		 			.style("font-size", fontSize + "px")
					.style("fill", "#333")
					.style("stroke-width", "0px");
					
		var red_pt = this.valueToPoint(configuration.min_red, 0.63);
		this.body.append("svg:text")
		 			.attr("x", red_pt.x)
		 			.attr("y", red_pt.y)
		 			.attr("dy", fontSize / 3)
		 			.attr("text-anchor", "start")
		 			.text(configuration.min_red)
		 			.style("font-size", fontSize + "px")
					.style("fill", "#333")
					.style("stroke-width", "0px");
		...
		pointerContainer.selectAll("text")
							...
							.enter()
								.append("svg:text")
									...
									.style("fill", "#000")
									.style("stroke-width", "0px");
		// TODO: add 'days' text below value
		this.redraw(value);
	};
	...
	this.redraw = function(value)
	{
		...
		pointer.transition()
					...
					.attrTween("transform", function()
					{
						...
						if (value > self.config.max) pointerValue = self.config.max + 0.01*self.config.range;
						else if (value < self.config.min) pointerValue = self.config.min - 0.01*self.config.range;
