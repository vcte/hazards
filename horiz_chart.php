...
<?php
  ...
  foreach ($dept_and_pi_with_most_hazards as $row) {
     $dept = $row[0];
     $pi_name = $row[1];
     $n_hazards = $row[2];
     if ($n_hazards > 0) {
...
?>
  ...
  y.domain(data.map(function(d) { return d.Dept; }));
  ...
  var g2 = g.append("g")
    ...
    .data(data)
    .enter().append("g")
      .attr("transform", function(d) { return "translate(0, " + y(d.Dept) + ")"; });
  g2.append("rect")
      .attr("x", 0)
      //.attr("y", function(d) { return y(d.Dept); })
      .attr("width", function(d) { return x(+d.Hazards); })
      .attr("height", y.bandwidth())
      .attr("fill", function(d) { return z(y(d.Dept)); })
